...
class ModelsBooks extends Model {
    ...
    public function createBook($data){
        $sql = "INSERT INTO " . DB_PREFIX ."books (`title`, `description`, `publication_date`, `isbn`, `date`)
         VALUES ('$data->title', '$data->description', '$data->publication_date', '$data->isbn', '$data->date')";
        $result = $this->db->query($sql);
        return ($result->num_rows > 0) ? true : false;
    }

    public function updateBook($data){
        $sql = "UPDATE " . DB_PREFIX ."books  SET `title` = '$data->title', `description` = '$data->description', 
        `publication_date` = '$data->publication_date', `isbn` = '$data->isbn', `date` = '$data->date' 
        WHERE `id` = '$data->id'";
        $result = $this->db->query($sql);
        return ($result->num_rows > 0) ? true : false;
    }
    ...
    public function deleteBook($id){
        $sql = "DELETE FROM " . DB_PREFIX . "books WHERE id =".$id;
        $result = $this->db->query($sql);
        return ($result->num_rows > 0) ? true : false;
